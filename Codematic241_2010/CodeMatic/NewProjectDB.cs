...
namespace Codematic
{
	...
	public partial class NewProjectDB : Form
	{
		...
		Thread mythread;
        LTP.IDBO.IDbObject dbobj;//数据库对象
        LTP.CodeBuild.CodeBuilders cb;//代码生成对象
        LTP.CmConfig.DbSettings dbset;//服务器配置        
        ...
        void ThreadWork()
		{
            ...
            for (int i=0;i<tblCount;i++)
			{
                ...
                //命名规则处理
                cb.ModelName = namerule.GetModelClass(strmodelname);
                cb.BLLName = namerule.GetBLLClass(strmodelname);
                cb.DALName = namerule.GetDALClass(strmodelname);


                DataTable dtkey = dbobj.GetKeyName(dbname, tablename);
                List<ColumnInfo> dt = dbobj.GetColumnInfoList(dbname, tablename);
                cb.Fieldlist = dt;
                cb.Keys = LTP.CodeHelper.CodeCommon.GetColumnInfos(dtkey);
                CreatCS();	
